...
`build-index.json` OCI referrer from each component image (per the reduced
snapshot spec) and runs `slan-cuan generate-security-metadata` (fath-cuan
process_osv) with OSIDB enrichment. This task is OSV-only — it does not
publish anything to Pulp; publishing is handled by a downstream task.
...
| Name               | Description                                                                    |
|--------------------|---------------------------------------------------------------------------------|
| sourceDataArtifact | Produced trusted data artifact, now also containing `security_metadata/*.json` |
